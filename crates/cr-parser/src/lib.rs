//! Language parsers and adapters for CR-SemService
//!
...
pub use cr_core::{Language, Result, AstNode, LanguageParser};
...
impl LanguageParserRegistry {
    ...
    pub fn parse_file(&self, file_path: &Path, source: &str) -> Result<Box<dyn AstNode>> {
        ...
        if let Some(parser) = self.get_parser(language) {
            ...
            Err(cr_core::AnalysisError::unsupported_language(format!(
    ...
    pub fn detect_language(&self, file_path: &Path) -> Result<Language> {
        if let Some(extension) = file_path.extension().and_then(|e| e.to_str()) {
            match extension.to_lowercase().as_str() {
                ...
                "c" | "h" => Ok(Language::C),
                _ => Err(cr_core::AnalysisError::unsupported_language(format!(
                    "Unsupported file extension: {}",
                    extension
                ))),
            }
        } else {
            Err(cr_core::AnalysisError::unsupported_language(

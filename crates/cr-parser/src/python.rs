...
impl PythonAdapter {
    ...
    fn parse_import_statement(&self, source: &str, _context: &AdapterContext) -> Result<UniversalNode> {
        if source.starts_with("from ") {
            ...
            if let Some(import_pos) = source.find(" import ") {
                ...
                Err(cr_core::AnalysisError::parse_error("Invalid from import statement"))
            }
            ...
            Err(cr_core::AnalysisError::parse_error("Invalid import statement"))
        }
    }
    ...
    fn parse_if_statement(&self, source: &str, _context: &AdapterContext) -> Result<UniversalNode> {
        if source.starts_with("if ") {
            if let Some(colon_pos) = source.find(':') {
                ...
                Err(cr_core::AnalysisError::parse_error("Invalid if statement"))
            }
            ...
            if let Some(colon_pos) = source.find(':') {
                ...
                Err(cr_core::AnalysisError::parse_error("Invalid elif statement"))
            }
        } else {
            // else:
    ...
    fn parse_loop_statement(&self, source: &str, _context: &AdapterContext) -> Result<UniversalNode> {
        if source.starts_with("for ") {
            if let Some(colon_pos) = source.find(':') {
                ...
                Err(cr_core::AnalysisError::parse_error("Invalid for statement"))
            }
            ...
            if let Some(colon_pos) = source.find(':') {
                ...
                Err(cr_core::AnalysisError::parse_error("Invalid while statement"))
            }
        } else {
            Err(cr_core::AnalysisError::parse_error("Unknown loop statement"))
        }
    }
    ...
    fn parse_try_statement(&self, source: &str, _context: &AdapterContext) -> Result<UniversalNode> {
        if source.starts_with("try:") {
            ...
            Err(cr_core::AnalysisError::parse_error("Unknown try statement"))
        }
    }
}

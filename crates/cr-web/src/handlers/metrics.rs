...
async fn generate_prometheus_metrics() -> String {
    ...
    // Service info
    metrics.push_str("# HELP cr_semservice_info Information about the CR-SemService instance\n");
    metrics.push_str("# TYPE cr_semservice_info gauge\n");
    metrics.push_str(&format!(
        "cr_semservice_info{{version=\"{}\"}} 1\n",
        env!("CARGO_PKG_VERSION")
    ));

    // Uptime
    metrics.push_str("# HELP cr_semservice_uptime_seconds Total uptime of the service in seconds\n");
    metrics.push_str("# TYPE cr_semservice_uptime_seconds counter\n");
    metrics.push_str(&format!("cr_semservice_uptime_seconds {}\n", get_uptime_seconds()));

    // Request metrics
    metrics.push_str("# HELP cr_semservice_requests_total Total number of HTTP requests\n");
    metrics.push_str("# TYPE cr_semservice_requests_total counter\n");
    metrics.push_str(&format!("cr_semservice_requests_total{{method=\"GET\",endpoint=\"/api/v1/health\"}} {}\n", get_request_count("GET", "/api/v1/health")));
    metrics.push_str(&format!("cr_semservice_requests_total{{method=\"POST\",endpoint=\"/api/v1/analyze\"}} {}\n", get_request_count("POST", "/api/v1/analyze")));
    metrics.push_str(&format!("cr_semservice_requests_total{{method=\"GET\",endpoint=\"/api/v1/metrics\"}} {}\n", get_request_count("GET", "/api/v1/metrics")));

    // Analysis metrics
    metrics.push_str("# HELP cr_semservice_analyses_total Total number of code analyses performed\n");
    metrics.push_str("# TYPE cr_semservice_analyses_total counter\n");
    ...
    for (language, count) in analysis_counts.iter() {
        metrics.push_str(&format!("cr_semservice_analyses_total{{language=\"{}\"}} {}\n", language, count));
    }

    // Analysis duration (real histogram would require more sophisticated tracking)
    let stats = collector.get_analysis_stats();
    metrics.push_str("# HELP cr_semservice_analysis_duration_seconds Time spent on code analysis\n");
    metrics.push_str("# TYPE cr_semservice_analysis_duration_seconds histogram\n");
    ...
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_bucket{{le=\"0.1\"}} {}\n", bucket_01));
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_bucket{{le=\"0.5\"}} {}\n", bucket_05));
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_bucket{{le=\"1.0\"}} {}\n", bucket_10));
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_bucket{{le=\"5.0\"}} {}\n", bucket_50));
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_bucket{{le=\"+Inf\"}} {}\n", total_analyses));
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_sum {:.1}\n", total_analyses as f64 * 0.5));
    metrics.push_str(&format!("cr_semservice_analysis_duration_seconds_count {}\n", total_analyses));

    // Findings metrics
    metrics.push_str("# HELP cr_semservice_findings_total Total number of findings detected\n");
    metrics.push_str("# TYPE cr_semservice_findings_total counter\n");
    metrics.push_str(&format!("cr_semservice_findings_total{{severity=\"info\"}} {}\n", get_findings_count("info")));
    metrics.push_str(&format!("cr_semservice_findings_total{{severity=\"warning\"}} {}\n", get_findings_count("warning")));
    metrics.push_str(&format!("cr_semservice_findings_total{{severity=\"error\"}} {}\n", get_findings_count("error")));
    metrics.push_str(&format!("cr_semservice_findings_total{{severity=\"critical\"}} {}\n", get_findings_count("critical")));

    // Job metrics
    metrics.push_str("# HELP cr_semservice_jobs_active Number of currently active jobs\n");
    metrics.push_str("# TYPE cr_semservice_jobs_active gauge\n");
    metrics.push_str(&format!("cr_semservice_jobs_active {}\n", get_active_jobs()));

    metrics.push_str("# HELP cr_semservice_jobs_total Total number of jobs processed\n");
    metrics.push_str("# TYPE cr_semservice_jobs_total counter\n");
    metrics.push_str(&format!("cr_semservice_jobs_total{{status=\"completed\"}} {}\n", get_job_count("completed")));
    metrics.push_str(&format!("cr_semservice_jobs_total{{status=\"failed\"}} {}\n", get_job_count("failed")));

    // Success rate
    metrics.push_str("# HELP cr_semservice_job_success_rate Job success rate percentage\n");
    metrics.push_str("# TYPE cr_semservice_job_success_rate gauge\n");
    metrics.push_str(&format!("cr_semservice_job_success_rate {:.2}\n", stats.success_rate()));

    // System metrics (real-time)
    metrics.push_str("# HELP cr_semservice_memory_usage_bytes Current memory usage in bytes\n");
    metrics.push_str("# TYPE cr_semservice_memory_usage_bytes gauge\n");
    metrics.push_str(&format!("cr_semservice_memory_usage_bytes {}\n", get_memory_usage()));

    metrics.push_str("# HELP cr_semservice_cpu_usage_percent Current CPU usage percentage\n");
    metrics.push_str("# TYPE cr_semservice_cpu_usage_percent gauge\n");
    metrics.push_str(&format!("cr_semservice_cpu_usage_percent {:.2}\n", get_cpu_usage()));

    // Rules metrics
    metrics.push_str("# HELP cr_semservice_rules_loaded Number of rules currently loaded\n");
    metrics.push_str("# TYPE cr_semservice_rules_loaded gauge\n");
    metrics.push_str(&format!("cr_semservice_rules_loaded {}\n", get_loaded_rules_count()));

    // Performance metrics
    metrics.push_str("# HELP cr_semservice_avg_findings_per_analysis Average findings per analysis\n");
    metrics.push_str("# TYPE cr_semservice_avg_findings_per_analysis gauge\n");
    metrics.push_str(&format!("cr_semservice_avg_findings_per_analysis {:.2}\n", stats.avg_findings_per_analysis()));
...
mod tests {
    ...
    async fn test_generate_prometheus_metrics() {
        ...
        assert!(metrics.contains("cr_semservice_info"));
        assert!(metrics.contains("cr_semservice_uptime_seconds"));
        assert!(metrics.contains("cr_semservice_requests_total"));
        assert!(metrics.contains("cr_semservice_analyses_total"));
        assert!(metrics.contains("cr_semservice_findings_total"));
        assert!(metrics.contains("cr_semservice_jobs_active"));
        assert!(metrics.contains("cr_semservice_memory_usage_bytes"));
        ...
        assert!(metrics.contains("cr_semservice_jobs_active 2"));
        assert!(metrics.contains(&format!("cr_semservice_info{{version=\"{}\"}} 1", env!("CARGO_PKG_VERSION"))));
    }

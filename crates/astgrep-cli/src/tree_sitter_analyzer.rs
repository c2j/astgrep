...
use astgrep_core::{Confidence, Finding, Language, Location, Result, Severity};
...
impl TreeSitterAnalyzer {
    ...
    pub fn apply_rule_with_tree_sitter(
        ...
        if let Some(tree) = self.parser.parse(source_code, language)? {
            for pattern in patterns {
                let matches = self
                    .parser
                    .find_pattern_matches(&tree, source_code, pattern)?;
    ...
    pub fn supports_language(&self, language: Language) -> bool {
        matches!(
            language,
            Language::Python | Language::JavaScript | Language::Java | Language::Bash
        )
    }
}

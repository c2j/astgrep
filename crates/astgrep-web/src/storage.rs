...
impl Storage for MemoryStorage {
    ...
    async fn list_jobs(&self, filter: &JobFilter) -> WebResult<Vec<Job>> {
        ...
        if let Some(ref status) = filter.status {
            filtered_jobs
                .retain(|job| format!("{:?}", job.status).to_lowercase() == status.to_lowercase());
        }
        ...
        let paginated_jobs = filtered_jobs.into_iter().skip(offset).take(limit).collect();
...
impl SqliteStorage {
    ...
    pub async fn new(database_url: &str) -> WebResult<Self> {
        let pool = sqlx::SqlitePool::connect(database_url).await.map_err(|e| {
            WebError::internal_server_error(format!("Failed to connect to database: {}", e))
        })?;
        ...
        sqlx::migrate!("./migrations")
            ...
            .await
            .map_err(|e| {
                WebError::internal_server_error(format!("Failed to run migrations: {}", e))
            })?;
...
impl Storage for SqliteStorage {
    async fn store_job(&self, job: &Job) -> WebResult<()> {
        let metadata_json = serde_json::to_string(&job.metadata).map_err(|e| {
            WebError::internal_server_error(format!("Failed to serialize metadata: {}", e))
        })?;
    ...
    async fn get_job(&self, job_id: Uuid) -> WebResult<Option<Job>> {
        let row = sqlx::query!("SELECT * FROM jobs WHERE id = ?1", job_id.to_string())
        ...
        if let Some(row) = row {
            ...
            let job = Job {
                id: Uuid::parse_str(&row.id).map_err(|e| {
                    WebError::internal_server_error(format!("Invalid job ID: {}", e))
                })?,
                status,
    ...
    async fn list_jobs(&self, filter: &JobFilter) -> WebResult<Vec<Job>> {
        ...
        if let Some(ref status_filter) = filter.status {
            jobs.retain(|job| {
                format!("{:?}", job.status).to_lowercase() == status_filter.to_lowercase()
            });
        }
        ...
        let paginated_jobs = jobs.into_iter().skip(offset).take(limit).collect();
    ...
    async fn delete_job(&self, job_id: Uuid) -> WebResult<()> {
        ...
        sqlx::query!(
            "DELETE FROM analysis_results WHERE job_id = ?1",
            job_id.to_string()
        )
        .execute(&self.pool)
        .await
    ...
    async fn store_results(&self, job_id: Uuid, results: &AnalysisResults) -> WebResult<()> {
        let results_json = serde_json::to_string(results).map_err(|e| {
            WebError::internal_server_error(format!("Failed to serialize results: {}", e))
        })?;
    ...
    async fn get_results(&self, job_id: Uuid) -> WebResult<Option<AnalysisResults>> {
        ...
        if let Some(row) = row {
            let results: AnalysisResults = serde_json::from_str(&row.results).map_err(|e| {
                WebError::internal_server_error(format!("Failed to deserialize results: {}", e))
            })?;
            Ok(Some(results))
        } else {
            Ok(None)
    ...
    async fn cleanup_old_jobs(
        ...
        let result = sqlx::query!("DELETE FROM jobs WHERE created_at < ?1", cutoff_time)
            ...
            .await
            .map_err(|e| {
                WebError::internal_server_error(format!("Failed to cleanup jobs: {}", e))
            })?;
...
mod tests {
    use super::*;
    use crate::models::{AnalysisSummary, JobStatus};
    use std::collections::HashMap;

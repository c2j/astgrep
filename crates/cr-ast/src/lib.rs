//! Universal AST definitions for CR-SemService
//!
...
impl UniversalAst {
    ...
    pub fn node_count(&self) -> usize {
        let mut count = 0;
        let _ = cr_core::ast_utils::visit_nodes(&self.root, &mut |_| {
            count += 1;
            Ok(())
        });
        count
    ...
    pub fn find_nodes_by_type(&self, node_type: &str) -> Vec<Box<dyn AstNode>> {
        cr_core::ast_utils::find_nodes(&self.root, &|node| {
    ...
    pub fn get_node_types(&self) -> Vec<String> {
        ...
        let _ = cr_core::ast_utils::visit_nodes(&self.root, &mut |node| {

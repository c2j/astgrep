...
impl TreeSitterParser {
    ...
    fn match_method_call(&self, node: &Node, source: &str, object: &str, method: &str) -> Result<bool> {
        ...
        if node.kind() == "method_invocation" {
            // For Java method invocations, check the method name
            if let Some(name_node) = node.child_by_field_name("name") {
                ...
                // Otherwise, check if the object matches
        ...
        if matches!(node.kind(), "call_expression" | "call") {
            ...
            if let Some(function_node) = node.child_by_field_name("function") {
                if matches!(function_node.kind(), "attribute" | "member_expression" | "field_expression") {
                    ...
                    // If object is a metavariable, match any object with the specified method
                    if object.starts_with('$') {
                        // Extract the method name from the function text

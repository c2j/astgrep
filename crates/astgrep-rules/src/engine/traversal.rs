...
impl RuleExecutionEngine {
    ...
    fn effective_sql_stmt_boundary(rule: &Rule, ctx: &RuleContext) -> bool {
        ...
        if let Some(v) = rule
            .get_metadata_string("sql_statement_boundary")
            .and_then(|s| parse_bool_like(&s))
        {
            return v;
        }
        if let Some(v) = ctx
            .get_data("sql_statement_boundary")
            .and_then(|s| parse_bool_like(s))
        {
            return v;
        }
    ...
    fn execute_pattern(
        ...
        if let PatternType::Regex(ref regex_str) = &pattern.pattern_type {
            match Regex::new(regex_str) {
                Ok(re) => {
                    ...
                    println!(
                        "🔍 Regex pattern execution complete. Generated {} findings",
                        findings.len()
                    );
                    return Ok(findings);
                }
        ...
        if let PatternType::Simple(ref pattern_str) = &pattern.pattern_type {
            ...
            if requires_advanced {
                if rule.requires_symbolic_propagation() {
                    println!(
                        "🔍 Pattern requires symbolic propagation, using AdvancedRuleExecutor"
                    );
                } else {
                    println!(
                        "🔍 Pattern has {} conditions, using AdvancedRuleExecutor",
                ...
                println!(
                    "🔍 AdvancedRuleExecutor found {} findings",
                    result.findings.len()
                );
            ...
            println!(
                "🔍 Pattern execution complete. Generated {} findings",
                findings.len()
            );
            return Ok(findings);
        }
        ...
        if let PatternType::All(ref subs) = &pattern.pattern_type {
            ...
            println!(
                "🔍 AdvancedRuleExecutor found {} findings for All pattern",
                result.findings.len()
            );
            return Ok(result.findings);
        }
        ...
        if let PatternType::Either(ref subs) = &pattern.pattern_type {
            ...
            for sub in subs {
                match &sub.pattern_type {
                    PatternType::Regex(r) => {
                        if let Ok(re) = Regex::new(r) {
                            for m in re.find_iter(&context.source_code) {
                                ...
                                let matched_text = &context.source_code
                                    [start_byte..end_byte.min(context.source_code.len())];
                    ...
                    PatternType::Simple(s) => {
                        ...
                        println!(
                            "DEBUG either: simple pattern '{}' produced {} spans",
                            s,
                            spans.len()
                        );
                        for (start_byte, end_byte) in spans {
                    ...
                    PatternType::All(_) => {
                        ...
                        if let Ok(result) = advanced_executor.execute_comprehensive_analysis(
                            ...
                            println!(
                                "🔍 AdvancedRuleExecutor found {} findings for All sub-pattern",
                                result.findings.len()
                            );
                            for finding in result.findings {
                                ...
                                let start_byte = self.line_col_to_byte_index(
                                    &context.source_code,
                                    start_line,
                                    start_col,
                                );
                                let end_byte = self.line_col_to_byte_index(
                                    &context.source_code,
                                    end_line,
                                    end_col,
                                );
            ...
            println!(
                "🔍 pattern-either execution complete. Generated {} findings",
                findings.len()
            );
        ...
        let matches =
            self.find_pattern_matches(pattern, _ast, context.language, &context.source_code)?;
    ...
    fn create_best_location_from_node_or_pattern(
        &self,
        node: &dyn AstNode,
        pattern: &Pattern,
        context: &RuleContext,
    ...
    fn find_pattern_matches(
        ...
        match &pattern.pattern_type {
            crate::types::PatternType::All(sub_patterns) => {
                println!(
                    "🔍 Processing All pattern with {} sub-patterns",
                    sub_patterns.len()
                );
                ...
                let positive_matches =
                    self.find_pattern_matches(&sub_patterns[0], ast, language, source_code)?;
                println!(
                    "🔍 All pattern: first sub-pattern found {} matches",
                    ...
                    for neg_pattern in &sub_patterns[1..] {
                        match &neg_pattern.pattern_type {
                            crate::types::PatternType::Not(inner) => {
                                ...
                                if let Some(neg_pattern_str) = inner.get_pattern_string() {
                                    ...
                                    if self.simple_pattern_match(
                                        ...
                                        println!(
                                            "🔍 Not excluded (doesn't match negative pattern)"
                                        );
                                    }
                                } else {
                                ...
                                if !self.evaluate_conditions_on_match(
                                    &neg_pattern.conditions,
                                    pos_match.as_ref(),
                                    source_code,
                                ) {
                                    all_satisfied = false;
                            ...
                            _ => {
                                ...
                                let neg_matches = self.find_pattern_matches(
                                    neg_pattern,
                                    ast,
                                    language,
                                    source_code,
                ...
                println!(
                    "🔍 All pattern: found {} matches after filtering sub-patterns",
                    matches.len()
                );
                ...
                if !pattern.conditions.is_empty() {
                    println!(
                        "🔍 Evaluating {} conditions on outer pattern",
                        pattern.conditions.len()
                    );
                    matches.retain(|m| {
                        let result = self.evaluate_conditions_on_match(
                            &pattern.conditions,
                            m.as_ref(),
                            source_code,
                        );
            ...
            crate::types::PatternType::Either(sub_patterns) => {
                println!(
                    "🔍 Processing Either pattern with {} sub-patterns",
                    sub_patterns.len()
                );
                ...
                for (i, sub_pattern) in sub_patterns.iter().enumerate() {
                    ...
                    let sub_matches =
                        self.find_pattern_matches(sub_pattern, ast, language, source_code)?;
                    println!(
                        "🔍 Either sub-pattern {} found {} matches",
                        i + 1,
                        sub_matches.len()
                    );
                ...
                astgrep_core::ast_utils::visit_nodes(ast, &mut |node| {
                    ...
                    if let Some(text) = node.text() {
                        ...
                        if let Some(pattern_str) = pattern.get_pattern_string() {
                            ...
                            println!(
                                "🔍 No pattern string found for pattern: {:?}",
        ...
        println!(
            "🔍 AST traversal complete. Visited {} nodes, found {} matches",
            node_count,
            matches.len()
        );
        Ok(matches)
                ...
                Condition::MetavariableType(metavar_type) => {
                    ...
                    if let Some(node_text) = matched_node.text() {
                        ...
                        if let Some(var_name) = node_text.split('.').next() {
                            ...
                            let decl_pattern =
                                format!(r"(\w+)\s+{}\s*=[^;]*;", regex::escape(var_name));
    ...
    fn try_match_tokens(
        ...
        text_tokens: &[(String, usize, usize)],
        start: usize,
        ...
        while i < pattern_tokens.len() {
            ...
            if case_insensitive {
                println!(
                    "TRACE try_match: i={}, j={}, p_tok='{}', text='{}'",
                    i, j, p_tok, text_tokens[j].0
                );
            }
            ...
            let is_dollar_ellipsis =
                p_tok == "$" && (i + 1) < pattern_tokens.len() && pattern_tokens[i + 1] == "...";

            // Ellipsis: match variable-length sequence (including empty) until next anchor
            if p_tok == "..." || is_dollar_ellipsis {
                if case_insensitive {
                    println!(
                        "TRACE ellipsis encountered at i={}, j={}, is_dollar_ellipsis={}",
                        i, j, is_dollar_ellipsis
                    );
                }
                // When consuming `$ ...`, advance pattern by 2 tokens; otherwise by 1
                if is_dollar_ellipsis {
                    i += 1;
                } // so the common handling below will also `i += 1` at the end
                ...
                match next_anchor_idx {
                    None => {
                        if case_insensitive {
                            println!("TRACE ellipsis to end: returning len={}", text_tokens.len());
                        }
                    ...
                    Some(k) => {
                        ...
                        if case_insensitive {
                            println!(
                                "TRACE ellipsis next anchor literal='{}' (k={})",
                                next_lit, k
                            );
                        }
                        ...
                        if next_lit == ")" || next_lit == "]" || next_lit == "}" {
                            ...
                            if let Some(end_pos) = set_pos {
                                if case_insensitive {
                                    println!("TRACE ellipsis matched to close at pos={}", end_pos);
                                }
                            ...
                            while pos < text_tokens.len() {
                                ...
                                let matched = if case_insensitive {
                                    tt.eq_ignore_ascii_case(next_lit)
                                } else {
                                    tt == next_lit
                                };
                                if matched {
                            ...
                            if let Some(end_pos) = found {
                                if case_insensitive {
                                    println!("TRACE ellipsis skipped to anchor at pos={}", end_pos);
                                }
                ...
                let next_lit_idx = (i + 1..pattern_tokens.len())
                    .find(|&k| pattern_tokens[k] != "..." && !pattern_tokens[k].starts_with('$'));
                match next_lit_idx {
                    None => {
                        let capture: Vec<String> =
                            text_tokens[j..].iter().map(|t| t.0.clone()).collect();
                        if capture.is_empty() {
                            return None;
                        }
                    ...
                    Some(k) => {
                        ...
                        if next_lit == ")" {
                            ...
                            if let Some(end_pos) = end_pos {
                                ...
                                let capture: Vec<String> = text_tokens[j..end_pos]
                                    .iter()
                                    .map(|t| t.0.clone())
                                    .collect();
                                if let Some(prev) = bindings.get(p_tok) {
                ...
                if !direct_match && !constant_prop_match {
                    return None;
                }
                i += 1;
                j += 1;
    ...
    fn find_pattern_spans_in_source(
        &self,
        pattern: &str,
        source: &str,
        language: astgrep_core::Language,
        ...
        println!(
            "DEBUG find_pattern_spans_in_source: pattern='{}', preprocessed='{}', lang={:?}",
            pattern, preprocessed, language
        );
        ...
        while idx < pattern_tokens.len() {
            if pattern_tokens[idx] == "$"
                && idx + 1 < pattern_tokens.len()
                && pattern_tokens[idx + 1] == "..."
            {
                coalesced.push("...".to_string());
                idx += 2;
            } else {
            ...
            match (first_anchor_idx, first_anchor.as_ref()) {
                (Some(anchor_idx), Some(anchor_tok)) => {
                    ...
                    for pos in 0..window.len() {
                        ...
                        let lit_ok = if case_insensitive {
                            tok.eq_ignore_ascii_case(anchor_tok)
                        } else {
                            tok == anchor_tok
                        };
                        if !lit_ok {
                        ...
                        if matches!(language, astgrep_core::Language::Java) {
                            if let Some(first_lit) =
                                pattern_tokens.iter().find(|t| !t.starts_with('$'))
                            {
                                let is_ident =
                                    first_lit.chars().all(|c| c.is_alphanumeric() || c == '_');
                                if is_ident
                                    && rel_start + win_start > 0
                                    && text_tokens[rel_start + win_start - 1].0 == "."
                                {
                                    continue;
                                }
                            }
                        }
                        if let Some(rel_end) = self.try_match_tokens(
                            &pattern_tokens,
                            window,
                            rel_start,
                            case_insensitive,
        ...
        let try_match_from = |start: usize| -> bool {
            ...
            while i < pattern_tokens.len() {
                ...
                if p_tok == "..." || is_dollar_ellipsis {
                    ...
                    let next_anchor_idx = (i + 1..pattern_tokens.len()).find(|&k| {
                        pattern_tokens[k] != "..." && !pattern_tokens[k].starts_with('$')
                    });
                    match next_anchor_idx {
                        None => {
                        ...
                        Some(k) => {
                            ...
                            if next_lit == ")" || next_lit == "]" || next_lit == "}" {
                                ...
                                while pos < text_tokens.len() {
                                    let matched = if case_insensitive {
                                        text_tokens[pos].eq_ignore_ascii_case(next_lit)
                                    } else {
                                        &text_tokens[pos] == next_lit
                                    };
                                    if matched {
                                        found_k = Some(pos);
                                        break;
                    ...
                    // 查找下一个字面量（非 $ / 非 ...）
                    let next_lit_idx = (i + 1..pattern_tokens.len()).find(|&k| {
                        pattern_tokens[k] != "..." && !pattern_tokens[k].starts_with('$')
                    });
                    match next_lit_idx {
                        None => {
                    ...
                    let matched = if case_insensitive {
                        text_tokens[j].eq_ignore_ascii_case(p_tok)
                    } else {
                        &text_tokens[j] == p_tok
                    };
                    if !matched {
    ...
    fn execute_dataflow(
        ...
        let sources_strings: Vec<String> = dataflow
            .sources
            .iter()
            .filter_map(|sp| {
            ...
            .collect();
        let sinks_strings: Vec<String> = dataflow
            .sinks
            .iter()
            .filter_map(|sp| {
                if let PatternType::Simple(s) = &sp.pattern.pattern_type {
        ...
        if !sources.is_empty() && !sinks.is_empty() {
            for sink in sinks {
                ...
                let finding = Finding::new(
                    rule.id.clone(),
                    format!(
                        "Potential data flow from source to sink: {}",
                        rule.description
                    ),
                    rule.severity,
                    rule.confidence,
    ...
    fn execute_taint_mode(
        ...
        // Use AdvancedRuleExecutor for pattern-based taint analysis
        use crate::executor::AdvancedRuleExecutor;
        ...
        let findings = executor.execute_taint_analysis(
            ...
            ast,
            None,
            Some(Path::new(&context.file_path)),
        )?;

        println!("🔍 Taint analysis found {} findings", findings.len());
    ...
    fn is_safe_boolean_sink(&self, line: &str) -> bool {
        let line_trimmed = line.trim();

        if line_trimmed.contains("!=") || line_trimmed.contains("==") {
            println!(
                "🔍 Safe boolean pattern found: comparison operator in line: '{}'",
                line_trimmed
            );
            return true;
        }

        if line_trimmed.contains(".valueOf(") || line_trimmed.contains(".parseBoolean(") {
            println!(
                "🔍 Safe boolean pattern found: Boolean conversion in line: '{}'",
                line_trimmed
            );
            return true;
        }

        if line_trimmed.contains(".isTrue(")
            || line_trimmed.contains(".isFalse(")
            || line_trimmed.contains(".isValid(")
        {
            println!(
                "🔍 Safe boolean pattern found: Boolean method in line: '{}'",
                line_trimmed
            );
            return true;
        }
    ...
    fn is_safe_number_sink(&self, line: &str) -> bool {
        ...
        if line_trimmed.contains("Integer.valueOf(")
            || line_trimmed.contains("Integer.parseInt(")
            || line_trimmed.contains("Long.valueOf(")
            || line_trimmed.contains("Long.parseLong(")
            || line_trimmed.contains("Short.valueOf(")
            || line_trimmed.contains("Short.parseShort(")
            || line_trimmed.contains("Double.valueOf(")
            || line_trimmed.contains("Double.parseDouble(")
            || line_trimmed.contains("Float.valueOf(")
            || line_trimmed.contains("Float.parseFloat(")
        {
            println!(
                "🔍 Safe number pattern found: numeric conversion in line: '{}'",
                line_trimmed
            );
            return true;
        }
        ...
        if line_trimmed.contains(".compareTo(")
            || line_trimmed.contains(".indexOf(")
            || line_trimmed.contains(".lastIndexOf(")
            || line_trimmed.contains(".length()")
        {
            println!(
                "🔍 Safe number pattern found: string method returning int in line: '{}'",
                line_trimmed
            );
            return true;
        }
        ...
        if line_trimmed.contains(".length") && !line_trimmed.contains(".length()") {
            println!(
                "🔍 Safe number pattern found: array length in line: '{}'",
                line_trimmed
            );
            return true;
        }
    ...
    fn is_complex_expression_sink(&self, line: &str) -> bool {
        ...
        if line_trimmed.contains("String.format(") {
            println!(
                "🔍 Complex expression sink found: String.format in line: '{}'",
                line_trimmed
            );
            return true;
        }
    ...
    fn execute_taint_mode_fallback(
        ...
        let assume_safe_booleans =
            ...
            } else if let Some(val) = dataflow.taint_assume_safe_booleans {
                val
            } else {
                false
            };
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if line.contains("sink(") {
                ...
                if let Some(col) = line.find("sink(") {
                    let location = Location::new(
                        PathBuf::from(&context.file_path),
                        line_num,
                        col + 1,
                    ...
                    .with_metadata(
                        "safe_booleans_filtered".to_string(),
                        assume_safe_booleans.to_string(),
                    );

                    findings.push(finding);
                    println!("🔍 Taint vulnerability found at line {}!", line_num);
                }
            }
        }
...
mod tests {
    ...
    fn test_dataflow_rule() {
        let mut engine = RuleExecutionEngine::new();
        let dataflow =
            DataFlowSpec::from_strings(vec!["input".to_string()], vec!["output".to_string()]);
    ...
    fn test_java_simple_with_metavar_multiple_occurrences() {
        ...
        .add_pattern(Pattern::simple(
            "response.getWriter().write($INPUT)".to_string(),
        ));
...
response.getWriter().write(\"<script>var data = '\" + scriptParam + \"';</script>\");\n";
    ...
    fn test_java_ellipsis_block_bodies() {
        ...
        let java_code =
            "class D{ void f(){ try { a(); b(); } catch (Exception e) { handle(); } } }";
        let ast = create_test_ast();
        let context = RuleContext::new(
            "Demo.java".to_string(),
            Language::Java,

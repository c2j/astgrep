...
use anyhow::Result;
use cr_core::{Language, OutputFormat, Severity, Confidence};
...
fn analyze_file_simple(
    ...
    if !config.rule_files.is_empty() {
        ...
        let file_findings = analyze_with_basic_patterns(file_path, &source_code, language, config)?;
        findings.extend(file_findings);
        stats.rules_executed += config.rule_files.len();
    } else {
...
fn analyze_with_basic_patterns(
    ...
    info!("Loaded {} rules for {:?}", rules.len(), language);
...
fn apply_metavariable_pattern(
    ...
    use cr_matcher::AdvancedPatternMatcher;
...
fn apply_enhanced_pattern_matching(
    ...
    use cr_rules::{RuleParser, Rule};
    use cr_matcher::AdvancedSemgrepMatcher;
    use cr_parser::tree_sitter_parser::TreeSitterParser;
...
fn convert_parsed_rule_to_yaml(rule: &ParsedRule) -> Result<String> {
    let mut yaml = format!(
        "rules:\n  - id: {}\n    name: {}\n    description: {}\n    severity: {:?}\n    languages: [",
        rule.id, rule.message, rule.message, rule.severity
    );
...
/// Convert our Pattern to SemgrepPattern
fn convert_pattern_to_semgrep_pattern(pattern: &cr_rules::Pattern) -> Result<cr_core::SemgrepPattern> {
    use cr_core::{SemgrepPattern, PatternType as CorePatternType};

    let core_pattern_type = match &pattern.pattern_type {
        cr_rules::PatternType::Simple(s) => CorePatternType::Simple(s.clone()),
        cr_rules::PatternType::Either(patterns) => {
            let converted: Result<Vec<_>> = patterns.iter()
                .map(convert_pattern_to_semgrep_pattern)
                .collect();
            CorePatternType::Either(converted?)
        }
        cr_rules::PatternType::Inside(inner) => {
            CorePatternType::Inside(Box::new(convert_pattern_to_semgrep_pattern(inner)?))
        }
        cr_rules::PatternType::NotInside(inner) => {
            CorePatternType::NotInside(Box::new(convert_pattern_to_semgrep_pattern(inner)?))
        }
        cr_rules::PatternType::Not(inner) => {
            CorePatternType::Not(Box::new(convert_pattern_to_semgrep_pattern(inner)?))
        }
        cr_rules::PatternType::Regex(regex) => CorePatternType::Regex(regex.clone()),
        cr_rules::PatternType::NotRegex(regex) => CorePatternType::NotRegex(regex.clone()),
        cr_rules::PatternType::All(patterns) => {
            let converted: Result<Vec<_>> = patterns.iter()
                .map(convert_pattern_to_semgrep_pattern)
                .collect();
            CorePatternType::All(converted?)
        }
        cr_rules::PatternType::Any(patterns) => {
            let converted: Result<Vec<_>> = patterns.iter()
                .map(convert_pattern_to_semgrep_pattern)
                .collect();
...
fn generate_text_output(
    ...
    output.push_str("=== CR-SemService Analysis Results ===\n\n");
        ...
        "runs": [{
            "tool": {
                "driver": {
                    ...
                    "informationUri": "https://github.com/your-org/cr-semservice"
                }
            },
...
fn generate_html_output(
    ...
    html.push_str("<title>CR-SemService Analysis Report</title>\n");
    html.push_str("<style>\n");
    ...
    html.push_str("<h1>CR-SemService Analysis Report</h1>\n");
...
fn generate_markdown_output(
    ...
    md.push_str("# CR-SemService Analysis Report\n\n");
...
fn generate_semgrep_compatible_output(
    ...
    // Semgrep header
    writeln!(&mut output, "┌──── ○○○ ────┐")?;
    writeln!(&mut output, "│ CR-SemService │")?;
    writeln!(&mut output, "└─────────────┘")?;
    writeln!(&mut output)?;

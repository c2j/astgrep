...
pub struct RuleEditor {
    ...
    parsed_rules: Vec<astgrep_rules::Rule>,
...
impl RuleEditor {
    ...
    /// Get parsed rules
    pub fn get_parsed_rules(&self) -> &[astgrep_rules::Rule] {
        &self.parsed_rules
    }
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                if let Some(rule) = self.parsed_rules.first() {
                        ...
                        for (i, pattern) in rule.patterns.iter().enumerate() {
                            ui.horizontal(|ui| {
                                ...
                                match &pattern.pattern_type {
                                    astgrep_rules::PatternType::Simple(pattern_str) => {
                    ...
                    ui.label(
                        "No valid rule parsed. Please check the YAML syntax in the Advanced tab.",
                    );
                }
            });
    }
        ...
        match self.rule_parser.parse_yaml(&self.content) {
            Ok(rules) => {
                ...
                if self.parsed_rules.is_empty() {
                    self.validation_errors
                        .push("No valid rules found in YAML".to_string());
                }
            }
            Err(e) => {
                self.validation_errors
                    .push(format!("Rule parsing error: {}", e));
            }
        }
    }

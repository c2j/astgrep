        ...
        ui.horizontal(|ui| {
            ...
            if !findings.is_empty() {
                let error_count = findings
                    .iter()
                    .filter(|f| matches!(f.severity, astgrep_core::Severity::Error))
                    .count();
                let warning_count = findings
                    .iter()
                    .filter(|f| matches!(f.severity, astgrep_core::Severity::Warning))
                    .count();
                let info_count = findings
                    .iter()
                    .filter(|f| matches!(f.severity, astgrep_core::Severity::Info))
                    .count();

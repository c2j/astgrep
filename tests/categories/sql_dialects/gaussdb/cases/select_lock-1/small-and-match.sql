...
-- @desc PACKAGE BODY 内多语句 RMW 模式检测（性能回归用小文件）
-- @expect MATCH
...
CREATE OR REPLACE PACKAGE BODY pkg_salary_update AS
    ...
    BEGIN
        ...
        SELECT cnt INTO v_updated_count FROM accounts7 WHERE id = 1 FOR UPDATE;
        v_updated_count := v_updated_count + 1;
        UPDATE accounts7 SET cnt = v_updated_count WHERE id = 1;
    ...
    v_updated_count1 INTEGER := 0;
    BEGIN
    SELECT cnt INTO v_updated_count1 FROM accounts7 WHERE id = 1 FOR UPDATE;
    v_updated_count1 := v_updated_count1 + 1;
    UPDATE accounts7 SET cnt = v_updated_count1 WHERE id = 1;
        -- 仅对绩效排名前N的员工调薪
        UPDATE emp_salary

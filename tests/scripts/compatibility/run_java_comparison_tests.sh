...
find_java_tests() {
    ...
    # Find all Java files in tests/categories/rules directory
    local java_files=($(find tests/categories/rules -name "*.java" | sort))
...
run_cr_semservice_test() {
    ...
    if [ -f "target/debug/astgrep" ] || [ -f "target/release/astgrep" ]; then
        # Use our CLI tool
        # echo "cargo run --bin astgrep -- analyze --config "$yaml_file" "$java_file" --format json"
...
EOF
    ...
    local taint_tests=$(find tests/categories/rules -name "taint_*.java" | wc -l)
    local metavar_tests=$(find tests/categories/rules -name "metavar_*.java" | wc -l)
    local sym_prop_tests=$(find tests/categories/rules -name "sym_prop_*.java" | wc -l)
    local cp_tests=$(find tests/categories/rules -name "cp_*.java" | wc -l)
...
run_sample_comparisons() {
    ...
    local sample_files=(
        "tests/categories/rules/taint_final_globals.java"
        "tests/categories/rules/metavar_comparison_bitxor.java"
        "tests/categories/rules/sym_prop_class_attr.java"
        "tests/categories/rules/cp_private_class_attr.java"
        "tests/categories/rules/taint_lambda1.java"
    )
...
main() {
    ...
    echo ""
    echo "📊 Quick Summary:"
    local java_count=$(find tests/categories/rules -name "*.java" | wc -l)
    local yaml_count=$(find tests/categories/rules -name "*.yaml" | wc -l)

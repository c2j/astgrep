...
# JavaScript Rules Comparison Test Script for astgrep vs Semgrep
# This script compares results between Semgrep and astgrep on JavaScript test files

set -e

echo "🔍 astgrep vs Semgrep JavaScript Rules Comparison"
echo "=================================================="
echo ""
...
# Find all JavaScript test files and their corresponding YAML rules
find_js_tests() {
    # Find all JavaScript files in tests/categories/rules directory
    local js_files=($(find tests/categories/rules -maxdepth 1 -name "*.js" | sort))
...
# Run Semgrep on a specific JavaScript file with its YAML rule
run_semgrep_test() {
    local js_file=$1
    local yaml_file=$2
    ...
    local semgrep_output=$(semgrep --config "$yaml_file" "$js_file" --json 2>/dev/null || echo '{"results":[]}')
...
# Run astgrep on a specific JavaScript file with its YAML rule
run_astgrep_test() {
    local js_file=$1
    local yaml_file=$2
    ...
    if [ -f "target/debug/astgrep" ] || [ -f "target/release/astgrep" ]; then
        local output=$(cargo run --bin astgrep -- analyze --config "$yaml_file" "$js_file" --format json 2>/dev/null || echo '{"findings":[]}')
...
compare_test_case() {
    local js_file=$1
    local base_name=$(basename "$js_file" .js)
    ...
    if [ ! -f "$yaml_file" ]; then
        print_status "WARNING" "No corresponding YAML rule found for $js_file"
        echo ""
        return 1
    fi
    ...
    echo "  JavaScript code preview:"
    head -10 "$js_file" | sed 's/^/    /'
    if [ $(wc -l < "$js_file") -gt 10 ]; then
        echo "    ... (truncated)"
    fi

    # Run tests
    print_status "INFO" "Running Semgrep..."
    local semgrep_matches=$(run_semgrep_test "$js_file" "$yaml_file")

    print_status "INFO" "Running astgrep..."
    local astgrep_matches=$(run_astgrep_test "$js_file" "$yaml_file")
    ...
    if [[ "$semgrep_matches" =~ ^[0-9]+$ ]] && [[ "$astgrep_matches" =~ ^[0-9]+$ ]]; then
        if [ "$semgrep_matches" -eq "$astgrep_matches" ]; then
            ...
            echo "      semgrep --config \"$yaml_file\" \"$js_file\" --json"
            echo "      cargo run --bin astgrep -- analyze --config \"$yaml_file\" \"$js_file\" --format json"
            echo ""
            return 1
        fi
    else
...
generate_comparison_report() {
    local js_files=($(find_js_tests))
    local total_tests=${#js_files[@]}
    ...
    local report_file="JAVASCRIPT_COMPARISON_REPORT.md"

    cat > "$report_file" << EOF
# JavaScript Rules Comparison Report
...
### Overview
- **Total JavaScript test files**: $total_tests
...
EOF
    ...
    local taint_tests=$(find tests/categories/rules -maxdepth 1 -name "taint_*.js" | wc -l)
    local metavar_tests=$(find tests/categories/rules -maxdepth 1 -name "metavar_*.js" | wc -l)
    local sym_prop_tests=$(find tests/categories/rules -maxdepth 1 -name "sym_prop_*.js" | wc -l)
    local cp_tests=$(find tests/categories/rules -maxdepth 1 -name "cp_*.js" | wc -l)
    local record_tests=$(find tests/categories/rules -maxdepth 1 -name "record_*.js" | wc -l)
    local vardef_tests=$(find tests/categories/rules -maxdepth 1 -name "vardef_*.js" | wc -l)
...
- **Record/object tests**: $record_tests files
- **Variable definition tests**: $vardef_tests files
...
EOF
    ...
    for js_file in "${js_files[@]}"; do
        ...
        if [ -f "$yaml_file" ]; then
            local rule_mode=$(grep -E "^\s*mode:" "$yaml_file" | head -1 | sed 's/.*mode:\s*//' | tr -d ' \n\r')
            local semgrep_matches=$(run_semgrep_test "$js_file" "$yaml_file" | tr -d '\n\r' | grep -o '[0-9]*' | head -1)
            local astgrep_matches=$(run_astgrep_test "$js_file" "$yaml_file" | tr -d '\n\r' | grep -o '[0-9]*' | head -1)
...
#### Record/Object Tests
Tests for JavaScript object handling.
Key patterns tested:
- Object spread
- Field ellipsis
- Record unification
...
Tests for variable definition matching.
Key patterns tested:
...
1. **Arrow Functions**: Support for () => {} syntax
2. **Template Literals**: Backtick strings
3. **Destructuring**: Object and array destructuring
4. **Optional Chaining**: ?. operator
5. **Nullish Coalescing**: ?? operator
6. **JSX Support**: React component patterns

#### Current Limitations
1. **JavaScript Parser Integration**: Need to verify tree-sitter JavaScript support
2. **Taint Analysis**: Advanced taint tracking for JavaScript patterns
...
5. **JSX/TSX Support**: Full React/TypeScript patterns
...
# Run comparison on all JavaScript tests
run_all_comparisons() {
    local js_files=($(find_js_tests))
    local total_tests=${#js_files[@]}
    ...
    print_status "INFO" "Running comparison on all $total_tests JavaScript test files..."
    echo ""

    for js_file in "${js_files[@]}"; do
        ((current_test++))
        echo "[$current_test/$total_tests]"
        if compare_test_case "$js_file"; then
            ((passed++))
        else
            ((failed++))
        fi
...
run_sample_comparisons() {
    ...
    local sample_files=(
        "tests/categories/rules/taint_lambda.js"
        "tests/categories/rules/sym_prop_react.js"
        "tests/categories/rules/record_spread_ellipsis.js"
        "tests/categories/rules/metavar_regex_id.js"
        "tests/categories/rules/js_constructor_naming.js"
    )

    local total_samples=${#sample_files[@]}
    local current_sample=0

    for js_file in "${sample_files[@]}"; do
        if [ -f "$js_file" ]; then
            ((current_sample++))
            echo "[$current_sample/$total_samples] Sample Test"
            compare_test_case "$js_file"
        fi
    done
...
show_sample_tests() {
    print_status "INFO" "Sample JavaScript test cases:"
    echo ""
...
main() {
    ...
    echo "Starting JavaScript rules comparison..."
    echo ""
    ...
    print_status "SUCCESS" "JavaScript comparison analysis completed!"
    echo ""
    echo "📊 Quick Summary:"
    local js_count=$(find tests/categories/rules -maxdepth 1 -name "*.js" | wc -l)
    ...
    echo "  - Test categories: Taint, Metavar, SymProp, ConstProp, Record, VarDef"
    echo ""
    echo "📄 Detailed report: JAVASCRIPT_COMPARISON_REPORT.md"
}
